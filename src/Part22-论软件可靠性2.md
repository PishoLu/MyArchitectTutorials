...
2023 年 3 月，我作为系统架构负责人，设计并开发了某公司的“材料科研论文智能解析平台”。该项目面对广大科研人员，服务涵盖全国范围，对系统的性能、可靠性要求很高，为确保质量，我贯彻可靠性设计的三原则，项目基于微服务架构，深度融合 Kubernetes、Istio 服务网格、Redis 集群、混合云及 CDN 等技术，构建了一套涵盖容错、检错、冗余与智能调度的可靠性保障体系。通过预设故障场景，采用无状态化设计、自动熔断与重试、弹性伸缩及多级监控等措施，实现了系统在组件故障、流量激增等异常情况下的高可用性与快速自愈。两年来的稳定运行证明，该可靠性设计有效保障了用户体验与业务连续性，为同类系统的架构设计提供了有价值的参考。

## 正文

我就职于一家为材料行业提供数字化服务软件的公司，我司与多个钢厂、材料研究室、高校合作多年。在长期的合作与发展下，深入了解行业痛点，结合最新的 AI 技术发展迅猛的势头，我司决心给行业科研人员提供一便捷、智能化的科研论文智能解析平台，该平台不仅收容了过去 50 年内的材料行业相关论文，通过 AI 对论文进行解析、分类、提取关键词，同时也支持在线对用户上传的文件进行文字提取、AI 解析、AI 知识库交互。该项目由用户与学者管理、文献管理、文献 AI 解析、智能算法推荐等多个子系统组成，在历经 8 个月的设计开发后，在 2023 年底上线，至今运行与维护长达两年，得到了行业内各人士的一致好评。我作为系统架构设计师，负责了该系统的设计与研发，该系统的可靠性设计是作为系统架构设计的核心目标之一，可靠性设计的合理决定了系统的可靠稳定，保障了系统的最终质量。

可靠性设计原则总结有三条，一、可靠性设计是软件设计的一部分，必须在软件的总体设计框架中使用。二、可靠性设计在满足提高软件质量的前提下，以提高和保障软件可靠性为最终目标。三、可靠性设计应确定软件的可靠性目标，不能无限扩大化，并且排在功能度、用户需求和研发费用之后考虑。

可靠性技术包含容错设计技术、检错技术、降低复杂度设计和系统配置技术等方面，在满足可靠性目标的前提下，不扩大化、并且排在功能度、用户需求和开发费用之后考虑，以提高和保障软件可靠性为最终目标。我们在“任何组件都会失效”的观念下，从架构层面预设各种故障场景（如服务器宕机、网络延迟、磁盘写满、依赖服务无响应等），设计时不在追求“永不故障的”的立项状态，而是聚焦于“故障发生时，系统如何自动应对，从而将影响降至最低”。这要求我们在设计阶段就进行全面的故障模式与影响分析。
...
通过系统性的可靠性设计，该项目在上线后经历了多次考验，取得了显著成效，用户评价为加载速度快、AI 解析精准深入、文献全面而实用。在这些背后，在遇到网络卡顿、云服务失效、数据库磁盘容量过大等多种问题时或有效切换到备用系统，或是自动扩容避免数据流失，或是优雅降级降低用户感知。该项目的缺点在于在项目初期投入巨大，需要提前向第三方购买文献数据、训练 AI、生成 AI 知识库，且混合云的安全问题相比公有云要复杂的多，如何保障私有数据不被泄露、确保私有云与公有云的通信安全是需要不断提出挑战解决问题的。文献数据作为敏感数据和静态数据，如果不使用 CDN，将会对服务器带来极大的带宽压力，为提高服务器可用性，同时保障敏感数据的安全性，我们使用了安全私有 CDN 的模式，配合文件服务器和认证机制，文献原文的 URL 具备服务器签名并指向私有 CDN， 仅有 15 分钟有效时间，同时文件服务器只接收 CDN 来源的请求，CDN 并不持久化缓存，这样虽然复杂，但是有效避免了数据大规模的泄露，同时使用 CDN 也有效的分担了这些静态资源的请求压力，整体是利大于弊的。
